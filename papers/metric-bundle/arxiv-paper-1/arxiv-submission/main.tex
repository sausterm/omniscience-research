...
For a physical (compact) gauge theory, we pass to the maximal compact
subgroup~\cite{Helgason1978}:
\begin{equation}
\label{eq:PS_chain}
...
The remainder of the paper establishes this result rigorously and explores its consequences.\looseness=-1
...
This is a \emph{geometric identity}---it holds exactly at every point of the section, with no undetermined coefficients.
The relative signs and unit coefficients are fixed by submanifold geometry (see, e.g.,~\cite{KobayashiNomizu}).

\subsection{From geometric identity to effective action}

The fourteen-dimensional Einstein-Hilbert action on~$Y$ is
\begin{equation}
\label{eq:14d_action}
S_Y = \frac{1}{16\pi G_{14}} \int_Y R_Y\, \mathrm{vol}_Y\,.
\end{equation}
To extract a four-dimensional effective action, we restrict to field configurations that are localised near the section $\sigma_g(X) \subset Y$.
This restriction is implemented by a \emph{section condition}: we consider only those fields on~$Y$ whose support is concentrated in a tubular neighbourhood of the section, and perform the fibre integral.

In the standard (compact) Kaluza-Klein setting, this fibre integral gives a finite volume factor $V_F$ that relates $G_{14}$ to the four-dimensional Newton constant: $G_4 = G_{14}/V_F$.
In the metric bundle, the fibre $\GL^+(4,\R)/\SO(3,1)$ is \emph{non-compact}, so the fibre volume diverges and a localisation mechanism is required.
Three approaches are available:

\begin{enumerate}[nosep, label=(\alph*)]
\item \emph{Gaussian localisation:} Fields decay as $\exp(-m^2\, d^2_{\mathrm{DW}}(y, \sigma_g(x)))$ in the fibre directions, where $d_{\mathrm{DW}}$ is the geodesic distance in the DeWitt metric.
The section is a critical point of the fibre potential, and the Gaussian integral is finite with effective volume $V_{\mathrm{eff}} = (\pi/m^2)^{5}$.
...
\item \emph{Section condition:} One restricts \emph{ab initio} to fields that depend only on the base coordinates~$x^\mu$, with the fibre dependence frozen to the background value.
This is the approach we adopt; it is analogous to the section condition in double field theory~\cite{HullZwiebach2009} and exceptional field theory~\cite{HohmSamtleben2013}.
\end{enumerate}

Under any of these approaches, the fibre integral produces an effective localisation factor~$V_{\mathrm{eff}}$ that determines the four-dimensional Newton constant $G_4 = G_{14}/V_{\mathrm{eff}}$, but does \emph{not} alter the relative coefficients in the Gauss equation~\eqref{eq:gauss}.
These relative coefficients---all unity---are fixed by the geometric identity and are not free parameters.

The resulting four-dimensional effective action is
\begin{equation}
\label{eq:action}
S_{\text{eff}} = \frac{1}{16\pi G_4} \int_X \Big[\, R_X \;+\; |H|^2 \;-\; |\mathrm{I\!I}|^2 \;+\; R^\perp \;+\; \text{(mixed)} \,\Big]\, \mathrm{vol}_X\,.
\end{equation}
The key point is that no undetermined ``$c_i$'' appear: the Gauss equation fixes all relative coefficients.
What remains undetermined is the overall scale $G_4$, which depends on the localisation mechanism.

\begin{remark}[Honest assessment of the localisation problem]
\label{rem:localisation}
The dynamical justification for the section condition is an open problem.
In standard compact KK theory, the localisation is automatic: the fibre is compact, and the zero-mode truncation is justified by energy scale separation.
For the non-compact fibre of the metric bundle, the section condition is an additional assumption whose consistency with the full 14D dynamics has not been established.
Approaches~(a) and~(b) are computationally tractable and give finite results, but require specifying the localisation scale~$m$ or the conformal damping rate, which introduces a parameter.
We regard this as the most significant open problem in the programme's dynamics.
\end{remark}
...
\begin{enumerate}
\item $+R_X$ is the \textbf{Einstein-Hilbert term} with the correct positive sign for attractive gravity.

\item $-|\mathrm{I\!I}|^2$ is a \textbf{torsion/extrinsic curvature term}. The negative sign ensures that the section tends to be totally geodesic, minimising extrinsic curvature.
At the trivial section over a flat base, $|\mathrm{I\!I}|^2 = 2$ and $|H|^2 = -1$ (the negative value arising from the indefinite DeWitt metric; see Paper~III~\cite{Paper3} for the explicit computation).

\item $R^\perp$ is the \textbf{Yang-Mills term}. The normal connection curvature $R^\perp$ is valued in $\mathfrak{so}(6,4)$; restricting to the maximal compact subalgebra gives the $\SU(4) \times \SU(2)_L \times \SU(2)_R$ gauge field strength~$F$.
By the Ricci equation, for gauge field perturbations $A^a_\mu$ along the Killing directions of $\mathfrak{o}(4)$:
\begin{equation}
R^\perp \;\supset\; -\frac{h_{ab}}{4}\, F^a_{\mu\nu} F^{b\,\mu\nu}\,,
\end{equation}
where the gauge kinetic metric $h_{ab}$ is determined by the DeWitt inner product on gauge orbits.
The critical result (proven in Paper~III, Theorem~4.1) is that $h_{ab}$ is \emph{positive definite}.
This follows because gauge transformations generated by $\mathfrak{o}(4)$ act via commutators $[k, h] = kh - hk$, which are automatically \emph{traceless}.
The DeWitt metric restricted to traceless symmetric tensors is positive definite (all nine eigenvalues are positive; only the trace direction is negative).
Therefore the Yang-Mills action has the correct sign with no ghost poles.
\end{enumerate}

All three sign tests---gravity attractive, torsion minimised, Yang-Mills ghost-free---are passed.
This is a non-trivial consistency check: the signs are determined by the DeWitt metric and are not adjustable.

\begin{remark}[Ghost-freedom beyond the gauge kinetic term]
\label{rem:ghosts}
The positive-definiteness of $h_{ab}$ establishes ghost-freedom at the level of the gauge kinetic term.
However, the full $\SO(6,4)$ structure means that the non-compact coset directions $\SO(6,4)/[\SO(6) \times \SO(4)]$ are present in the normal bundle.
These coset directions correspond to deformations of the signature structure (changing which subspace is positive-norm vs.\ negative-norm) and are frozen by the choice of Lorentzian background---they are not propagating fields.
Full non-perturbative unitarity of the fourteen-dimensional theory remains an open question.
\end{remark}
...
The effective volume factor $V_{\mathrm{eff}}$ relating $G_{14}$ to $G_4$ depends on the localisation mechanism for the non-compact fibre (see Remark~\ref{rem:localisation}).
...
Closing this gap requires a first-principles derivation of the localisation, which is the subject of ongoing work.
\end{enumerate}

\subsection{Distinguishing predictions}

The metric bundle framework makes several predictions that distinguish it from a \emph{generic} Pati-Salam model (in which the gauge group and representations are chosen by hand):
\begin{enumerate}[nosep]
\item \emph{Pati-Salam specifically:} The gauge group is $\SU(4) \times \SU(2)_L \times \SU(2)_R$, not $\SU(5)$ or $\SO(10)$.
This is a structural prediction, not an assumption.
\item \emph{Two Higgs Doublet Model:} The Higgs sector is the $(\mathbf{1}, \mathbf{2}, \mathbf{2})$ bidoublet, predicting $h^0, H^0, A^0, H^\pm$ beyond the SM Higgs.
\item \emph{Tree-level flat potential:} The quartic coupling is radiatively generated, with $\lambda(M_{\mathrm{PS}}) = g^2/4$ from Gauge-Higgs Unification.
\item \emph{Three generations from quaternionic structure:} $N_G = \dim(\mathrm{Im}(\mathbb{H})) = 3$ is derived, not input (Paper~V).
\item \emph{Cabibbo angle:} $\sin\theta_C = 1/\sqrt{2\,\dim(F)} = 1/\sqrt{20} = 0.224$, within $0.75\%$ of the observed value $0.225$ (Paper~V).
\end{enumerate}
These predictions are in principle falsifiable: discovery of a fundamental scalar in a representation other than $(\mathbf{1}, \mathbf{2}, \mathbf{2})$, or evidence for $\SU(5)$ proton decay modes, would rule out the framework.
